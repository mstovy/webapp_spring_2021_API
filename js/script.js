function searchGoogleBooks() {
    ...
    if (txtSearch.value == "") {
        ...
        document.querySelector("#mainDiv").innerHTML = "No search keyboard provided...";
        return
    }

    document.querySelector("mainDiv").classList.remove("hasError");

    fetch('http://www.googleapis.com/books/v1/volumes?&q=' + txtSearch.value + "&startIndex=0&manResults=20")
        .then(response => response.json())
        .then(json => {
            var list = '<div class="row col-sm-5">found ${json.totalItems} books</div> <div class="row" > '
            for (let i = 0; i < json.items.length; i++) {
                console.log(json.items[i].volumeInfor.title)
                ...
                template += '<li class="list-group-items">${bookInfor.authors}</li>'
                template += '<li class="list-group-item">${bookInfor.publisher}, ${bookInfo.publishedDate}</li>'
                template += '</ul>'
                list += template;
            }
            list += '</div>'
